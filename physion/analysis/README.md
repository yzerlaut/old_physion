# Analysis

*All analysis features are demonstrated in the following jupyter notebooks* 

- [** NWB file demo **](../../notebooks/NWB_file_demo.ipynb)

- [Orientation and direction selectivity to grating stimuli (demo on Ca-Imaging data)](../../notebooks/Direction-Selectivity.ipynb)

- [Phase estimate using Hilbert transform (demo on LFP-data)](../../notebooks/Hilbert_transform_and_phase_estimate.ipynb)

- [Wavelet Transform for time-frequency analysis (demo on LFP-data)](../../notebooks/Wavelets.ipynb)

- [Principal Components Analysis (demo on Ca-imaging data)](../../notebooks/PCA_demo.ipynb)
