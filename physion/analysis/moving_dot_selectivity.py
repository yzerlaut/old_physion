...
from analysis.orientation_direction_selectivity import orientation_selectivity_index, summary_fig
...
def analysis_pdf(datafile, iprotocol=0, Nmax=1000000):
    ...
    with PdfPages(pdf_filename) as pdf:
        ...
        for roi in np.arange(data.iscell.sum())[:Nmax]:
            ...
            SIs.append(orientation_selectivity_index(cell_resp['direction'][cell_resp['significant']],
                                                     cell_resp['value'][cell_resp['significant']]))
            if np.sum(cell_resp['significant'])>1:
                Nresp += 1
